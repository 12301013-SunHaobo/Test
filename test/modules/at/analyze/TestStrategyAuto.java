...
public class TestStrategyAuto {
	...
	private static boolean generateAllDatesChart = true;//true to generate all dates chart and save to tmp folder
	private static void testByDates() throws Exception{
		String stockCode = "qqq";//qqq, tna, tza 
		String[][] dateTimeArr = null;
			//if(generateAllDatesChart){
				dateTimeArr = initAllDates(stockCode); //all dates under data/naz/tick/output/qqq
			//} else {
		    //    dateTimeArr = initListedDate(); //listed dates only
			//}
		//get barList
		for(int i=0;i<dateTimeArr.length;i++){
			...
			boolean saveToFile = generateAllDatesChart;//save to file | display
